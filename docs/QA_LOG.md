...
- 当前 UI 使用本地 mock 搜索/RAG 数据和 React state 驱动可见交互；选题、文案、封面 Prompt 和 imagegen PNG 封面图通过本地 Codex CLI 真实生成。
...
- 生成选题、生成文案、生成封面 Prompt 和生成封面图都必须由用户点击触发，并在右侧显示 Codex CLI 调用状态。
